...
class SettingsFragment(
    ...
    private fun getDrawable(name: String): Drawable {
        val id = res.getIdentifier(name, "drawable", "com.MovieBox")
        return res.getDrawable(id, null) ?: throw Exception("Drawable $name not found")
    }

    private fun <T : View> View.findView(name: String): T {
        val id = res.getIdentifier(name, "id", "com.MovieBox")
        if (id == 0) throw Exception("View ID $name not found.")
        return this.findViewById(id)
    }

    private fun View.makeTvCompatible() {
        val outlineId = res.getIdentifier("outline", "drawable", "com.MovieBox")
    ...
    override fun onCreateView(
        ...
        savedInstanceState: Bundle?,
            ...
            val layoutId = res.getIdentifier("fragment_moviebox_settings", "layout", "com.MovieBox")
            val layout = res.getLayout(layoutId)
            val view = inflater.inflate(layout, container, false)
            
            val drawableId = res.getIdentifier("dialog_background", "drawable", "com.MovieBox")
        if (drawableId != 0) {
